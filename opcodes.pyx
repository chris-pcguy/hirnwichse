...
cdef class Opcodes:
    ...
    cdef int cmpsFuncWord(self, unsigned char operSize) except -1:
        ...
        for i in range(countVal):
            ...
            if (not dfFlag):
                esiVal = (esiVal+operSize)
                ediVal = (ediVal+operSize)
            else:
                esiVal = (esiVal-operSize)
                ediVal = (ediVal-operSize)
    ...
    cdef int xlatb(self) except -1:
        ...
        data = self.registers.mmReadValueUnsignedByte(<unsigned int>(baseValue+data), (<Segment>self.registers.segments.ds), True)
    ...
    cdef int interrupt(self, signed short intNum=-1, signed int errorCode=-1) except -1: # TODO: complete this!
        ...
        self.main.debug("Opcodes::interrupt: Go Interrupt {0:#04x}", intNum)
        if (self.registers.protectedModeOn):
