...
cdef class IsaDma:
    ...
    cdef void raiseHLDA(self):
        ...
        phyAddr = ((currChannel.page << 16) | \
                   (currChannel.currentAddress << ma_sl)&BITMASK_WORD)
        (<IsaDmaChannel>(<IsaDmaController>self.controller[ma_sl]).channel[channel]).DACK = True
        if (currChannel.addressDecrement):
            currChannel.currentAddress = (currChannel.currentAddress-1)&BITMASK_WORD
        else:
            currChannel.currentAddress = (currChannel.currentAddress+1)&BITMASK_WORD
        currChannel.currentCount = (currChannel.currentCount-1)&BITMASK_WORD
