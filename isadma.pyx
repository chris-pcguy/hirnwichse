...
cdef class IsaDma:
    ...
    cdef uint32_t inPort(self, uint16_t ioPortAddr, uint8_t dataSize) nogil:
        ...
        if (dataSize == OP_SIZE_WORD):
            ...
            self.main.exitError("ISADma::inPort: unknown dataSize. (ioPortAddr: 0x%04x, dataSize: %u)", ioPortAddr, dataSize)
            return 0
    ...
    cdef void outPort(self, uint16_t ioPortAddr, uint32_t data, uint8_t dataSize) nogil:
        ...
        if (dataSize == OP_SIZE_WORD):
            ...
            self.main.exitError("ISADma::outPort: unknown dataSize. (ioPortAddr: 0x%04x, data: 0x%04x, dataSize: %u)", ioPortAddr, data, dataSize)
            return
        elif (ioPortAddr in (0x00, 0x02, 0x04, 0x06, 0xc0, 0xc4, 0xc8, 0xcc)):
    ...
    cdef void setDRQ(self, uint8_t channel, uint8_t val):
        ...
        if (currChannel.transferMode not in (DMA_MODE_DEMAND, DMA_MODE_SINGLE, DMA_MODE_CASCADE)):
            self.main.exitError("ISADMA::setDRQ: transferMode %u not handled.", currChannel.transferMode)
            return
