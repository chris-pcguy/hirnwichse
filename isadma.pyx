...
cdef class Channel:
    ...
    cdef public int channelMasked, startAddress, transferBytes
...
cdef class Controller:
    ...
    cdef tuple channel
    cdef unsigned char flipFlop, transferModeChannel, transferModeTransferDirection, transferModeAutoInitialize, \
        transferModeCountDirection, transferModeMode, transferModeCountDirectionDown
    ...
    def setTransferMode(self, int transferModeByte):
    ...
    def maskChannel(self, unsigned char channelNum, unsigned char maskIt):
    ...
    def setAddrByte(self, unsigned char channelNum, unsigned char data):
        if (self.flipFlop):
            self.startAddress = ((data&0xff)<<8)|(self.startAddress&0xff)
        else:
            self.startAddress = (self.startAddress&0xff00)|(data&0xff)
    def setCountByte(self, unsigned char channelNum, unsigned char data):
        if (self.flipFlop):
